...
without its `overlay` short-circuit).
...
value outranks it. Stats (#161) was the same move: `--stats-accent`, the
value's ink, is keyed on `item` in the four colour-bearing skins, and only
the value reads it, so nothing on the root lost a property. Lynx needs nothing new either. Its runtime stamps axis classes
...
only defaults (Spinner's "Loading", now its hidden label's text) reads the
app's value before its default, and a reference list the part wires (a progressbar's
