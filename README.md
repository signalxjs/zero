...
| Package | What it is |
|---|---|
| [`@sigx/zero`](packages/zero) | The runtime foundation: anatomy contract, headless behaviors (controllable models, SSR-safe ids, roving focus, dismissal), 58 unstyled components (primitives plus the content, navigation and behavior tiers), theme engine, `manifest.json` + `llms.txt` |
...
| [`@sigx/create-zero-ds`](packages/create-zero-ds) | `pnpm create @sigx/zero-ds <name> --brief <id>` — scaffolds a design-system package from a style brief, with zero-basic's 52 recipes as the baseline fitted to the brief's vocabulary |
