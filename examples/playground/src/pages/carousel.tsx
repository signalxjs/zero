...
const CarouselDemos = component(() => {
    ...
    return () => (
        <>
            ...
            <p>
                Starting mid-way, with the model bound and written from outside:
                it opens on stop 2 and the buttons below move it. Only its own
                viewport scrolls — never the page (#171).
            </p>
            <Carousel.Root label="Tour" model={[tour, 'index']}>
                <Carousel.Viewport>
                    <Carousel.Item><div style={slideStyle}>Tour stop 1 — harbour</div></Carousel.Item>
                    <Carousel.Item><div style={slideStyle}>Tour stop 2 — old town</div></Carousel.Item>
                    <Carousel.Item><div style={slideStyle}>Tour stop 3 — castle</div></Carousel.Item>
                </Carousel.Viewport>
                <Carousel.IndicatorGroup>
                    <Carousel.Indicator index={0} label="Go to tour stop 1" />
                    <Carousel.Indicator index={1} label="Go to tour stop 2" />
                    <Carousel.Indicator index={2} label="Go to tour stop 3" />
                </Carousel.IndicatorGroup>
            </Carousel.Root>
            <p>
                <button type="button" onClick={() => { tour.index = 0; }}>Tour: first stop</button>{' '}
                <button type="button" onClick={() => { tour.index = 2; }}>Tour: last stop</button>
            </p>
        </>
    );
}, { name: 'CarouselDemos' });
