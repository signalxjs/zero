...
import type { TableSort } from '@sigx/zero';
...
interface Service { name: string; region: string; latency: number }

const SERVICES: readonly Service[] = [
    { name: 'search', region: 'us-east', latency: 87 },
    { name: 'api-gateway', region: 'eu-north', latency: 12 },
    { name: 'notifications', region: 'ap-south', latency: 143 },
    { name: 'billing', region: 'eu-west', latency: 31 },
];

/** The app owns the data: zero only reports which way to sort it. */
function sortServices(rows: readonly Service[], sort: TableSort | null): readonly Service[] {
    if (!sort) return rows;
    const key = sort.column as keyof Service;
    const sign = sort.direction === 'ascending' ? 1 : -1;
    return [...rows].sort((a, b) => (a[key] < b[key] ? -sign : a[key] > b[key] ? sign : 0));
}

const TableDemos = component(() => {
    const state = signal({ sort: { column: 'name', direction: 'ascending' } as TableSort | null });
    return () => (
        <>
            <p>
                ...
                of the page. A row can carry <code>data-selected</code>.
            </p>
            <Table.Root>
            ...
            <p>
                Sortable columns: a <code>sortable</code> header cell wraps its
                label in a real button and carries <code>aria-sort</code>; the
                root's <code>model:sort</code> says which column and which way, and
                the app re-orders its own rows from it. This one has the three-press
                cycle (<code>sortCycle="three"</code>) — the third press unsorts.
                Region does not sort.
            </p>
            <Table.Root model:sort={() => state.sort} sortCycle="three" columns={[
                { key: 'name', label: 'Service', sortable: true },
                { key: 'region', label: 'Region' },
                { key: 'latency', label: 'Latency', align: 'end', sortable: true },
            ]}>
                <Table.Caption>Services (sortable)</Table.Caption>
                <Table.Head />
                <Table.Body>
                    {sortServices(SERVICES, state.sort).map((row) => (
                        <Table.Row key={row.name}>
                            <Table.Cell column="name">{row.name}</Table.Cell>
                            <Table.Cell column="region">{row.region}</Table.Cell>
                            <Table.Cell column="latency">{row.latency} ms</Table.Cell>
                        </Table.Row>
                    ))}
                </Table.Body>
            </Table.Root>
            <p><small>Sort model: <code>{state.sort ? `${state.sort.column} ${state.sort.direction}` : 'none'}</code></small></p>
            <p>
