...
2. **Scaffold the package.** Run the scaffold rather than copying a layout
   ...
   <name>/
     ...
     src/baseline.ts     # @sigx/zero-basic's 52 recipes, copied whole — yours now
   ...
   **Conditional styles go in `at`** — the same shape, recursively:
   ...
   - **A state indicator is DRAWN GEOMETRY, and it must look different in
     ...
     1. **Every pair of a part's declared states must render differently.**
        ...
        how it looks. The opt-outs you author are `skipStates` (the state
        may look like any sibling) and the narrower `sameAs` (`{ root:
        { complete: 'loading' } }` — these two look alike on purpose, every
        other pair still differs); both are read **per part** (skipping
        `checked` on `item-label` will not excuse `item-indicator`) and both
        are design claims, so comment them. The manifest
   ...
   - **You are not limited to three axes.** `color`, `size` and `variant` have
     ...
     **This is also the answer when one component wants two vocabularies.**
     ...
     (A part whose anatomy declares `carries` — timeline's `marker`,
     steps' `item` and stats' `item` carry `color` — takes the SAME axis
     per instance: `<Timeline.Marker color="error">`,
     `<Steps.Item color="error">`, `<Stats.Item color="warning">`. That
...
| `state-legibility/disclosure` | error | the control of an in-flow disclosure (collapsible, accordion, tree-view) that says nothing about `open`/`closed` | `skipStates` or `sameAs` on the control; a sibling `*indicator` that differentiates |
