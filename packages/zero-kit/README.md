...
`src/button.ts`), `@sigx/zero-basic`'s 52 recipes as `src/baseline.ts`, and a
...
Lynx parses hex, `rgb()` and `hsl()` only, so the lynx target bakes every
other colour function — `oklch()`, `light-dark()`, `color-mix()` — to a hex
