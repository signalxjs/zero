...
export function compileDesignSystem<R extends RolesDecl, T extends SystemTokens>(
    ...
    // Table's stacked geometry (zero#55): zero's, in `zero.structure`, but
    // per breakpoint — so the kit writes it from this design system's ramp.
    // Into the table's own stylesheet when the skin styles the table (a
    // consumer importing per component still gets it), else straight into
    // the index. Only while the manifest's table declares the attribute.
