 ...
 * and self-layered (`@layer zero.recipes`), and the file opens with the
 * shared layer-order statement, so load order does not matter — and
 * re-emitting `tokens.css` would duplicate the design system's `@property`
 * registrations and drift its bytes across kit versions.
 *
 ...
 * self-layered (`@layer zero.recipes`), and the file opens with
 * `LAYER_ORDER_STATEMENT` — without it, importing this file first would
 * create `zero.recipes` before `zero.fallback`/`zero.tokens` and put the
 * recipes BELOW them (#180). With it, the file may be imported in any order
 * relative to the design system's own stylesheet and base.css.
 */
